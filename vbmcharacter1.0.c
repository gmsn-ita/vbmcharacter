/*BY FILIPE MATUSALEM, APR 2017     filipematus@gmail.com version1.0 */
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#include <string.h>
...
{
FILE *procar,*outcar;
int i,j,k,l,nbands,nkpts,nelect,nions,band,kpoint;
float valor[3];
char str[150],ch;
...
printf("Enter the number of the band and the number of the k-point as arguments for the program!\n\n");
exit(0);}
...
do
fscanf(procar,"%s",str);                                      /*posiciona o ponteiro após a palavra k-point selecionado*/
while(strcmp(str,"k-point")!=0);
}

fscanf(procar,"%s",str);fscanf(procar,"%s",str);     /*pula o número e o :*/
fscanf(procar,"%f",&valor[0]);fscanf(procar,"%f",&valor[1]);fscanf(procar,"%f",&valor[2]);
...
for(i=0;i<nions;i++){mtx[i][10]==0;
	for(j=0;j<10;j++){fscanf(procar,"%f",&mtx[i][j]);}
	do
...
printf("------------------------------------------------------------------------\n\n");
printf("ABSOLUTE CONTRIBUTIONS OF EACH ORBITAL FOR BAND %d IN K-POINT %f %f %f\n\n",band,valor[0],valor[1],valor[2]);
